...
class ConfigurableProxy extends EventEmitter {
  ...
  handleProxy(kind, req, res) {
    ...
    return this.targetForReq(req).then(match => {
      ...
      if (match.data && match.data.user && req.headers && req.headers.cookie) {
        xsrf[match.data.user] = this.getXsrf(req.headers.cookie);
      }

      //handle auth with _xsrf cookie
      if ("cookiecheck" in match.data && match.data.cookiecheck) {
        if (!req.headers || !req.headers.cookie) {
          that.handleProxyError(404, kind, req, res);
          return;
        } else {
          var currentXsrf = this.getXsrf(req.headers.cookie);
          var valid = false;
          for (var key in xsrf) {
            if (xsrf[key] === currentXsrf) {
              valid = true;
            }
          }
          if (!valid) {
