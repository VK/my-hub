...
from hyperopt.base import JOB_STATE_NEW, JOB_STATE_RUNNING, JOB_STATE_DONE, JOB_STATE_ERROR
...
from hyperopt.utils import working_dir, temp_dir
import six
...
def connection_with_tunnel(
    ...
    if ssh:
        ...
        connection = pymongo.MongoClient(host, port, document_class=SON, w=1, journal=True)
        if user:
            if not pw:
                pw = read_pw()
...
class MongoJobs:
    ...
    def insert(self, job):
        ...
        try:
            ...
            _id = self.jobs.insert_one(cpy, check_keys=True)
...
class MongoTrials(Trials):
    ...
    def count_by_state_unsynced(self, arg):
        ...
        rval = len(list(self.handle.jobs.find(query)))
        return rval
